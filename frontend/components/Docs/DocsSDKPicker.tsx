...
const FRAMEWORKS: { value: Framework; label: string; icon: React.ReactNode }[] =
  [
    {
      value: "React",
      label: "React",
...
export function shouldShowSDKPicker(pathname: string): boolean {
  return SDK_PICKER_PATHS.some((path) => pathname.startsWith(path));
}

export function DocsSDKPicker() {
  const { framework, setFramework } = useDocsPreferences();

  return (
    <div className="rounded-lg border border-border bg-muted/40 dark:bg-zinc-900 p-3 mb-6">
      <p className="text-xs text-muted-foreground mb-2">
        Customize your docs experience &mdash; choose your preferred framework
        for code examples:
      </p>
