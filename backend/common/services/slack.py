...
# Email patterns generated by automated test agents (e.g. Agent Score signup test).
# These create real accounts in production but shouldn't trigger team notifications.
_TEST_EMAIL_DOMAINS = frozenset([
    "score.trypillar.com",
])

# Our own domains — used to suppress Agent Score notifications for internal
# automated scans (batch refreshes, CI) that don't have a requester email.
...
def _is_test_email(email: str) -> bool:
    """Return True if the email belongs to an automated test agent."""
...
def notify_new_signup(
    ...
    if _is_test_email(email):
        logger.debug("Skipping Slack notification for test account: %s", email)
        return False
...
def notify_agent_score_complete(
    ...
    """
    if _is_test_email(email):
        logger.debug("Skipping Slack notification for test scan: %s", email)
        return False

    if domain.lower() in _OWN_DOMAINS and not email:
        logger.debug("Skipping Slack notification for internal self-scan: %s", domain)
        return False
