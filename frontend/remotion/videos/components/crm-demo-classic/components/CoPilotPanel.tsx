...
// Scene timing constants - aligned with CRM UI transitions
const TYPING_START = 50; // Start typing as soon as zoom begins
const TYPING_END = 149; // Typing finishes at ~125, then pause before send
const SEND_BUTTON_PRESS = 149; // When send button is pressed (longer pause after typing)
...
// Step timing - synced with CRM UI changes (4 steps with activeText/completeText)
const STEP_TIMING = [
  { start: 185, complete: 260 },  // Step 1: Searching → Found Walmart
  { start: 270, complete: 354 },  // Step 2: Setting stage → Closed Won
  { start: 364, complete: 420 },  // Step 3: Opening handoff → Opened form
  { start: 430, complete: 503 },  // Step 4: Notifying team → Notified
];

// Response timing - appears right after button click while form still shows "Sent!"
const RESPONSE_START = 505;
...
export const CoPilotPanel = ({
  ...
  // Scene states
  const isScene2OrLater = frame >= 50;
  ...
  // Calculate how many characters to show (Scene 3: frames 105-180)
      ...
      >
        <div
          style={{
            ...
            borderRadius: 15,
            border: `${showInputFocus ? "2px" : "0.8px"} solid ${showInputFocus ? "#F97316" : "#E4E0D9"}`,
            padding: "18px 24px",
            gap: 18,
            boxShadow: showInputFocus
              ? `0 0 0 ${4 * focusGlowOpacity}px rgba(249, 115, 22, ${0.15 * focusGlowOpacity})`
              : "none",
          }}
        >
          <div
          ...
          >
            <span style={{ display: "inline" }}>
              ...
              {showInputFocus && !isSpinning && (
                <BlinkingCursor frame={frame} startFrame={50} visible={true} />
              )}
            </span>
          </div>
          <div
            style={{
              ...
              height: 48,
              borderRadius: 12,
              backgroundColor: isSpinning ? "#22C55E" : "#F97316",
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
