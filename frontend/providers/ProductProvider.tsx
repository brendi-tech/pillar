...
import { adminFetch, adminPatch } from '@/lib/admin/api-client';
...
export function ProductProvider({ children }: ProductProviderProps) {
  ...
  const switchProduct = useCallback((productId: string) => {
    ...
    console.log('[ProductProvider] Switching to product:', product.name, productId);

    // 1. Update product selection
    setCurrentProductId(productId);

    // 2. Update organization if different
    if (product.organization_id && product.organization_id !== currentOrganizationId) {
      console.log('[ProductProvider] Switching organization to:', product.organization_id);
    ...
    // 4. Save to server (fire-and-forget, don't block UI)
    adminPatch('/users/me/preferences/', { last_selected_product_id: productId })
      .catch(err => console.warn('[ProductProvider] Failed to save preference to server:', err));
  }, [availableProducts, currentOrganizationId, setCurrentProductId, setCurrentOrganizationId, queryClient]);
  ...
  const refetchProducts = useCallback(async () => {
    ...
    try {
      ...
      const response = await adminFetch<{ results: AdminProduct[]; count: number }>('/configs/', {
        skipAutoContext: true
      });
      console.log('[ProductProvider] Fetched products:', response.results.length);
  ...
  useEffect(() => {
    if (hasRestoredFromServer || isLoading || availableProducts.length === 0 || !user) {
      return;
    }

    // If no product selected in localStorage but server has a preference, use it
    const localStorageProductId = typeof window !== 'undefined' 
      ? localStorage.getItem(CURRENT_PRODUCT_KEY) 
      : null;

    if (!localStorageProductId && user.last_selected_product_id) {
      const serverProduct = availableProducts.find(p => p.id === user.last_selected_product_id);
      if (serverProduct) {
        console.log('[ProductProvider] Restoring product from server preference:', user.last_selected_product_id);
        setCurrentProductId(user.last_selected_product_id);
        
        // Also sync organization
        if (serverProduct.organization_id && serverProduct.organization_id !== currentOrganizationId) {
          setCurrentOrganizationId(serverProduct.organization_id);
          if (typeof window !== 'undefined') {
            localStorage.setItem(CURRENT_ORG_KEY, serverProduct.organization_id);
          }
        }
      }
    }
  ...
  }, [hasRestoredFromServer, isLoading, availableProducts, user, currentOrganizationId, setCurrentProductId, setCurrentOrganizationId]);
  ...
  useEffect(() => {
    ...
    if (currentProductId) {
      ...
      if (exists) {
        console.log('[ProductProvider] Stored product is valid:', currentProductId);
        
        // Ensure organization matches the product's organization
        const product = availableProducts.find(p => p.id === currentProductId);
        if (product?.organization_id && product.organization_id !== currentOrganizationId) {
          console.log('[ProductProvider] Syncing organization to product\'s org:', product.organization_id);
          setCurrentOrganizationId(product.organization_id);
          if (typeof window !== 'undefined') {
            localStorage.setItem(CURRENT_ORG_KEY, product.organization_id);
          }
      ...
      console.log('[ProductProvider] Stored product no longer accessible, will use default');
    }

    // No valid stored product, use default or first available
    // Prefer a product from current organization if available
    const currentOrgProducts = availableProducts.filter(p => p.organization_id === currentOrganizationId);
    const productsToCheck = currentOrgProducts.length > 0 ? currentOrgProducts : availableProducts;
    
    const defaultProduct = productsToCheck.find(p => p.is_default);
    if (defaultProduct) {
      console.log('[ProductProvider] Setting default product:', defaultProduct.id);
      setCurrentProductId(defaultProduct.id);
      // Sync org if needed
      if (defaultProduct.organization_id && defaultProduct.organization_id !== currentOrganizationId) {
      ...
      console.log('[ProductProvider] Setting first available product:', productsToCheck[0].id);
      setCurrentProductId(productsToCheck[0].id);
      // Sync org if needed
      if (productsToCheck[0].organization_id && productsToCheck[0].organization_id !== currentOrganizationId) {
        setCurrentOrganizationId(productsToCheck[0].organization_id);
        if (typeof window !== 'undefined') {
          localStorage.setItem(CURRENT_ORG_KEY, productsToCheck[0].organization_id);
        }
      }
    }
  }, [isLoading, availableProducts, currentProductId, currentOrganizationId, setCurrentProductId, setCurrentOrganizationId]);
  ...
  const contextValue = useMemo(
    ...
    [currentProductId, currentProduct, availableProducts, switchProduct, setCurrentProductId, isLoading, refetchProducts]
  );
