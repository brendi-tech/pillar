...
def run(report: AgentScoreReport) -> list[CheckResult]:
    ...
    checks.append(_check_aria_labels(report.axe_results, report.accessibility_tree))
    checks.append(_check_landmark_roles(report.accessibility_tree))
    checks.append(_check_keyboard_focusable(report.axe_results))
    checks.append(_check_consistent_nav(report.accessibility_tree))
...
def _check_landmark_roles(ax_tree: dict) -> CheckResult:
...
def _check_keyboard_focusable(axe_results: dict) -> CheckResult:
...
def _check_consistent_nav(ax_tree: dict) -> CheckResult:
