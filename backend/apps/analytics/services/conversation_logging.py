...
class ConversationLoggingService:
    ...
    async def log_assistant_response(
        ...
        *,
        ...
        peak_context_occupancy: Optional[float] = None,
    ) -> str:
        """
        Finalize assistant message after streaming completes.
        
        Updates the pre-created assistant message (created in
        create_conversation_and_user_message) with final content and metadata.
        
        Args:
            conversation_id: Conversation UUID
            organization_id: Organization UUID
            product_id: Product UUID
            response: Assistant's final response text
            response_time_ms: Response generation latency in ms
            chunks_retrieved: List of RAG chunks [{chunk_id, score, source_article_id}]
            model_used: LLM model name
            message_id: Pre-generated assistant message UUID to update
            display_trace: Unified reasoning trace (thinking + tool steps interleaved)
            was_stopped: Whether user stopped streaming early
            tokens_generated: Tokens generated before stop
            prompt_tokens: Total input tokens across all LLM calls
            completion_tokens: Total output tokens across all LLM calls
            total_tokens: prompt_tokens + completion_tokens
            peak_context_occupancy: Peak context window usage percentage
        ...
        try:
            ...
            if updated:
                logger.debug(
                    f"[ConversationLogging] Finalized assistant message: "
                    f"conv={conversation_id}, assistant_msg={message_id}"
                )
            else:
                logger.warning(
                    f"[ConversationLogging] Assistant message not found for finalization: "
            ...
            logger.error(
                f"[ConversationLogging] Failed to finalize assistant message: {e}",
                exc_info=True,
            )
            raise

    @staticmethod
    async def flush_assistant_content(message_id: str, content: str) -> bool:
        """
        Progressively flush accumulated content to the assistant message.
        
        Called periodically (~every 2s) during streaming to keep the DB
        in sync with what has been sent to the client.
        
        Args:
            message_id: Pre-generated assistant message UUID
            content: Accumulated response content so far
        ...
        try:
            updated = await ChatMessage.objects.filter(
                id=message_id,
                role=ChatMessage.Role.ASSISTANT,
            ).aupdate(content=content)
            return bool(updated)
        except Exception as e:
            logger.warning(f"[ConversationLogging] Flush failed for {message_id}: {e}")
            return False
