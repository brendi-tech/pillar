...
// Pillar logo with name - extracted from the main codebase
const PillarLogoWithName = ({ light = false }: { light?: boolean }) => (
  <svg
...
export const PillarLogoOverlay = ({
  frame,
  type,
  introEnd,
  outroStart,
  totalFrames,
}: PillarLogoOverlayProps) => {
  let opacity = 0;

  if (type === "intro") {
    // Intro: logo visible briefly, then quick fade out (0.25 seconds = 8 frames)
    opacity = interpolate(frame, [0, introEnd - 4, introEnd], [1, 1, 0], {
      extrapolateLeft: "clamp",
      extrapolateRight: "clamp",
    });
  } else {
    // Outro: quick fade in (8 frames), then stays visible
    opacity = interpolate(
      frame,
      [outroStart, outroStart + 8, totalFrames],
      [0, 1, 1],
      {
        extrapolateLeft: "clamp",
        extrapolateRight: "clamp",
