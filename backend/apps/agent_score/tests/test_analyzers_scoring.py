...
Rules, signup_test, and openclaw contribute to the overall number.
...
class TestGetField:
    ...
    def test_dataclass_returns_attribute(self):
        c = _check("rules", 80, 1.0)
        assert _get_field(c, "category") == "rules"
        assert _get_field(c, "score") == 80
        assert _get_field(c, "weight") == 1.0

    def test_dict_returns_key(self):
        d = {"category": "rules", "score": 70, "weight": 2.0}
        assert _get_field(d, "category") == "rules"
...
class TestCalculateOverallScore:
    ...
    def test_mixed_scores(self):
        ...
        assert result["categories"]["rules"] == 50
        assert result["categories"]["signup_test"] == 60  # (80 + 40) / 2
        assert result["categories"]["webmcp"] == 60
        assert result["categories"]["openclaw"] == 70

        # Expected (webmcp excluded):
        # 50*0.25 + 60*0.25 + 70*0.50 = 12.5 + 15 + 35 = 62.5 → 62 (banker's rounding)
    ...
    def test_accepts_dicts(self):
        ...
        checks = [
            {"category": "rules", "score": 100, "weight": 1.0},
            {"category": "rules", "score": 100, "weight": 1.0},
        ]
        result = calculate_overall_score(checks)
        assert result["overall"] >= 0
        assert "rules" in result["categories"]
        assert result["categories"]["rules"] == 100
...
class TestSignupTestWeightRedistribution:
    ...
    def test_redistribution_changes_effective_weights(self):
        ...
        # Rules=100, WebMCP=0, Signup=0, Openclaw=0
        checks_with_signup = [
            _check("rules", 100, 1.0),
            _check("webmcp", 0, 1.0),
            _check("signup_test", 0, 1.0),
        ...
        # Rules=100, WebMCP=0, Openclaw=0 (no signup_test)
        ...
        # With signup: 100*0.25 + 0*0.25 + 0*0.50 = 25
        assert result_with["overall"] == 25
        # Without signup: rules=0.25/(0.25+0.50)=0.333 → 100*0.333 ≈ 33
        assert result_without["overall"] == 33

    def test_only_content_scores_correct_proportion(self):
        """With only rules checks, it should get 100% of the weight."""
        checks = [
            _check("rules", 80, 1.0),
        ]
        result = calculate_overall_score(checks)
        # Only rules exists, so it gets 100% of the weight
        assert result["overall"] == 80
        assert result["categories"]["rules"] == 80
    ...
    def test_signup_test_zeroed_is_different_from_absent(self):
        ...
        # Present but 0: 100*0.25 + 0*0.25 + 100*0.50 = 75
        assert result_present["overall"] == 75
        # Absent: rules+openclaw all 100 → 100
